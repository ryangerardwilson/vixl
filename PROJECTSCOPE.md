...
- DF navigation/editing: h/j/k/l, H/L (prev/next sheet for multi-sheet files), `:`, `i` (launch vim to edit current cell), `,conf` (open config in vim), `x`, `, i r a`, `, i r b`, `, d r`, `, i c a`, `, i c b`, `, d c`, `, r n c`, `, y a`, `, y c`, `,xr`, `,xar`, `,xc`, `,x+`, `,x-`, `, h`, `, l`, `, k`, `, j`, `, p j`. Expanded rows wrap on word boundaries (hard-break only for overlong words). `,xar` (expand all rows) computes row height using the maximum wrapped lines across all columns (including off-screen ones) to avoid horizontal-scroll "jerk". Vim-based editing works regardless of expansion state.
- Save-As flow: inline prompt on Ctrl+S/Ctrl+T when unsaved; validates .csv/.parquet/.xlsx/.h5; Ctrl+T exits only after successful save.
